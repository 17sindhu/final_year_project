...
# Remove ngrok browser warning page for all responses
@app.after_request
...
def _smart_predict(clean, model):
    ...
    Probability bars always match the final label shown.
    """
    kw = keyword_predict(clean)
    if kw:
        label, conf, _, _ = kw
        method = "keyword"
        # Build probability array that MATCHES the keyword label
        # so the chart is consistent with the displayed result
        remaining = round((1.0 - conf) / 2, 3)
        label_to_idx = {"Negative": 0, "Neutral": 1, "Positive": 2}
        proba = [remaining, remaining, remaining]
        proba[label_to_idx[label]] = round(conf, 3)
...
def api_bulk():
    ...
    for rev in reviews:
        ...
        db_save(rev, clean, model, f"bulk-{method}", label, conf, tokens)
